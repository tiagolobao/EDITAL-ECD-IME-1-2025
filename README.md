
# A. Raciocínio lógico e quantitativo: 

> Estatistica basica (Bussab)

 [1 - Proporções, porcentagens e variações percentuais](./A/a.md)

 [2 - Noções de média, mediana e desvio-padrão](./A/a.md)

 [3 - Interpretação de gráficos e tabelas](./A/a.md)

# B. Fundamentos de Estatística: 

> Estatistica basica (Bussab)

 [1 - Tipos de variáveis e escalas de medida.](./B/1.md)

 [2 - Medidas de tendência central e dispersão.](./B/2.md)

 [3 - Noções de correlação e regressão linear simples.](./B/3.md)

 [4 - Noções de probabilidade.](./B/4.md)

 [5 - Noções de intervalo de confiança e teste de hipóteses.](./B/5.md)

 [6 - Interpretação de gráficos (boxplot, histograma, diagrama de dispersão).](./B/6.md)


# C. Interpretação de resultados:

> Introducao a inferencia Estatistica

 [1 - Leitura de resultados de tabelas estatísticas (p-valor, IC, coeficientes, estatísticas).](./C/7.md)

 [2 - Escolha adequada de métodos ou visualizações.](./C/8.md)


# D. Noções de Computação:

> Livro datascience com R: https://r4ds.hadley.nz/

> NETTO, A.; MACIEL, F. Python para Data Science e Machine Learning Descomplicado. 1. ed

 [1 - Capacidade de interpretar comandos simples e seus resultados.](./D/1.md)

 [2 - Noções de manipulação de dados (importação, filtragem, resumo, visualização).](./D/2.md)

 [3 - Reconhecimento de funções básicas e objetos em R ou Python.](./D/3.md)

 [4 - Interpretação de mensagens de erro simples](./D/4.md)


# E. Aplicações e pensamento analítico:

> Nao tenho livro

 [1 - Como a Estatística pode apoiar decisões em áreas aplicadas.](./E/1.md)

 [2 - Interpretação crítica de estudos com dados.](./E/2.md)
